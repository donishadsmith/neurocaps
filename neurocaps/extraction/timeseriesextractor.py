    ...
    def get_bold(
        ...
        **This pipeline is most optimized for BOLD data preprocessed by fMRIPrep.**
        ...
        slice_time_ref: :obj:`int` or :obj:`float`, default=0.0
            The reference slice expressed as a fraction of the ``tr`` that is subtracted from condition onset times to
            adjust for slice time correction when ``condition`` is not None (``onset - slice_time_ref * tr``). Values
