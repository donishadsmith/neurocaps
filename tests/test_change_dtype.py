import numpy as np, os, glob, pandas as pytest
...
def test_change_dtype():
    ...
    changed_subject_timeseries = change_dtype(subject_timeseries_list=[subject_timeseries, subject_timeseries2], dtype="float16")
...
def test_change_dtype_w_pickle():
    changed_subject_timeseries = change_dtype(subject_timeseries_list=[os.path.join(os.path.dirname(__file__), "sample_timeseries.pkl")],
                                                  output_dir=os.path.dirname(__file__), dtype=np.float16, return_dicts=True)
    assert changed_subject_timeseries["dict_0"]["1"]["run-1"].dtype == "float16"
    changed_subject_timeseries = change_dtype(subject_timeseries_list=[os.path.join(os.path.dirname(__file__), "sample_timeseries.pkl")],
