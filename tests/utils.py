...
def check_filename_end(filenames):
    """Checks end of default filename."""
    for file in filenames:
        ext = os.path.splitext(file)[-1]
        assert ext
        ext = ext if not ext == ".gz" else ".nii.gz"
        stripped_file = file.removesuffix(ext)
        # Fail if `file` is filename..png or filename.png.png or "filename..png.png"
        assert stripped_file == stripped_file.rstrip(".") == stripped_file.removesuffix(ext)
...
def check_outputs(tmp_dir, values_dict, plot_type="map", plot_name=None):
    ...
    """
    min_plot_byte_size = 50000
    min_csv_byte_size = 50
    ...
    if plot_type == "map":
        ...
        check_filename_end(heatmap_files + outer_files)
        [os.remove(file) for file in heatmap_files + outer_files]
    elif plot_type == "radar":
        if "html" in values_dict:
            radar_htmls = glob.glob(os.path.join(tmp_dir.name, "*radar*.html"))
            assert len(radar_htmls) == values_dict["html"]
            assert all(os.path.getsize(x) >= min_plot_byte_size for x in radar_htmls)
            check_filename_end(radar_htmls)
            [os.remove(file) for file in radar_htmls]
        elif "json" in values_dict:
            radar_jsons = glob.glob(os.path.join(tmp_dir.name, "*radar*.json"))
            assert len(radar_jsons) == values_dict["json"]

            with open(radar_jsons[0], "r") as f:
                fig = json.load(f)
            ...
            check_filename_end(radar_jsons)
            [os.remove(file) for file in radar_jsons]
        else:
            radar_pngs = glob.glob(os.path.join(tmp_dir.name, "*radar*.png"))
            assert len(radar_pngs) == values_dict["png"]
            assert all(os.path.getsize(x) >= min_plot_byte_size for x in radar_pngs)
            check_filename_end(radar_pngs)
            [os.remove(file) for file in radar_pngs]
        ...
        assert all(os.path.getsize(x) >= min_plot_byte_size for x in png_files)
        assert all(os.path.getsize(x) >= min_csv_byte_size for x in csv_files)
        check_filename_end(png_files + csv_files)
        ...
        check_filename_end(pickle_files)
        [os.remove(file) for file in pickle_files]
    else:
        surface_pngs = glob.glob(os.path.join(tmp_dir.name, "*surface*.png"))
        assert len(surface_pngs) == values_dict["png"]
        assert all(os.path.getsize(x) >= min_plot_byte_size for x in surface_pngs)
        check_filename_end(surface_pngs)
        [os.remove(file) for file in surface_pngs]
