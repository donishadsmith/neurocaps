...
def test_calculate_methods():
    ...
    assert list(sorted_frequency_dict.values()) == [x for x in counts_df.loc[0,:].values if x != float("nan")]
    assert sum([x for x in counts_df.loc[0,:].values if x != float("nan")]) == first_subject_timeseries["1"]["run-1"].shape[0]

    proportion_dict = {num: value/len(first_subject_labels) for num, value in sorted_frequency_dict.items()}
    assert list(proportion_dict.values()) == [x for x in temp_df.loc[0,:].values if x != float("nan")]

    # Transition Frequency
    transition_frequency = np.where(np.diff(first_subject_labels) != 0,1,0).sum()
    ...
    assert list(persistence_dict.values()) == [x for x in persistence_df.loc[0,:].values if x != float("nan")]
